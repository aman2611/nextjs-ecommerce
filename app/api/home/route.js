...
export const GET = async (request) => {
    try {
        ...
        const bannerListProducts = await ProductModel.find({ _id: { $in: bannerList } }, {
            _id:1, title:1, rating:1, price:1, discount:1, mainImg:1, description:1,
            platform:1, genre:1, year:1, videoLinkKey:1, saved:1, library:1, developer:1,publisher:1,ageRating:1
        })    

        const sideSecListProducts = await ProductModel.find({ _id: { $in: sideSecList } }, {
        ...
        return new Response(JSON.stringify({
            ...
            sideSecProducts:sideSecListProducts,
